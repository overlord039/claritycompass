...
import { ClipboardCheck, FileText, CheckCircle2, RotateCcw } from 'lucide-react';
        ...
        const generateTasks = async () => {
            if (user && user.profile && user.lockedUniversities.length > 0 && user.applicationTasks.length === 0) {
                ...
                const result = await generateApplicationTasks({
                    ...
                    degree: user.profile.intendedDegree,
                    major: user.profile.fieldOfStudy,
                    sopStatus: user.profile.sopStatus as any,
                    ieltsStatus: user.profile.ieltsStatus as any,
                    greStatus: user.profile.greStatus as any,
                });
